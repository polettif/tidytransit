#' Conflicts between the tidytransit and other packages
#'
#' This function lists all the conflicts between packages in tidytransit
...
tidytransit_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", tidytransit_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "tidytransit_conflicts")
}

tidytransit_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "tidytransit_conflicts()"
  )
...
#' @export
print.tidytransit_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(tidytransit_conflict_message(x))
}
...
ls_env <- function(env) {
  x <- ls(pos = env)
  if (identical(env, "package:gtfsr")) {
    x <- setdiff(x, c("import_gtfs"))
  }
  x
}
